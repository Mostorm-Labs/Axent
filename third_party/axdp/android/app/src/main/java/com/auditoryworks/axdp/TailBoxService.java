...
public class TailBoxService {
    private static final String TAG = TailBoxService.class.getSimpleName();
    private AxdpService axdpService;
    private Context context;
    private USBHelper usbHelper;
    private Handler mHandler;
    private WifiHelper mWifiHelper;
    public TailBoxService(Context context, AxdpService axdpService, USBHelper usbHelper) {
        this.context = context;
        this.axdpService = axdpService;
    ...
    private int sendWifiSSId() {
        ...
        if (deviceHandle!=0){
            return 0;
            //return axdpService.nativeSetTailWifiSsid(deviceHandle, tailWifiJsonData, tailWifiJsonData.length);
        }else {
        ...
        if (usbDevice != null) {
            ...
            if (usbHelper.checkAndRequestPermission(usbDevice)) {
                if (!openUsbDevice(usbDevice, DeviceDesc.Tail_interfaceNum)) {
                    ...
                    if (result!=-1){
                        mHandler.postDelayed(() -> Toast.makeText(context.getApplicationContext(), "Tail box 已连接", Toast.LENGTH_SHORT).show(),1000);
                    }
    ...
    synchronized private boolean openUsbDevice(UsbDevice usbDevice, int interfaceNum) {
        ...
        deviceHandle = axdpService.nativeCreateDevice(fileDescriptor, usbDevice.getVendorId(), usbDevice.getProductId(), interfaceNum);
    ...
    synchronized private void closeUsbDevice() {
        if (deviceHandle != 0) {
            ...
            axdpService.nativeDestroyDevice(deviceHandle);
            deviceHandle = 0;
            mHandler.post(() -> Toast.makeText(context.getApplicationContext(), "Tail box 已断开", Toast.LENGTH_SHORT).show());
        }
    }
